...
def main():
    ...
    conversionScriptPath = os.path.abspath(os.path.join(args.output_dir, 'convertAI2RPR.ms'))
    shutil.copyfile(os.path.join(os.path.dirname(__file__), 'convertAI2RPR.ms'), os.path.join(args.output_dir, 'convertAI2RPR.ms'))
        ...
        if test['status'] == 'active':
            if os.path.exists(os.path.join(args.output_dir, test['name'] + '.render_time.log')):
                try:
                    ...
                    main_logger.error("Error {} during RPR log parsing: {}".format(str(err), test['name'] + '.render_time.log'))
